...
public class ServerNetworking {
        ...
        if (!user.getWorld().isClient && !user.hasStatusEffect(ModItems.TIEJI)) {
            ...
            if (stack.getItem() == SkillCards.GONGXIN) {
                ...
                if (cd > 0) user.sendMessage(Text.translatable("dabaosword.cooldown").formatted(Formatting.RED), true);
                    ...
                    openInv(user, target, Text.translatable("gongxin.title"), targetInv(target, false, false, 2, stack));
                    setCD(stack, 30);
                }
            }
            ...
            if (stack.getItem() == SkillCards.YIJI) {
                int i = getTag(stack);
                if (i > 0 ) openInv(user, target, Text.translatable("give_card.title", stack.getName()), targetInv(user, false, false, 2, stack));
            }

            if (stack.getItem() == SkillCards.RENDE) {
                openInv(user, target, Text.translatable("give_card.title", stack.getName()), targetInv(user, false, false, 2, stack));
            }
        }
    ...
    public static void active(PlayerEntity user, ItemStack stack) {
        if (!user.getWorld().isClient && !user.hasStatusEffect(ModItems.TIEJI)) {
            ...
            if (stack.getItem() == SkillCards.ZHIHENG) {
                int z = getTag(stack);
                if (z > 0) openInv(user, user, Text.translatable("zhiheng.title"), targetInv(user, true, false, 2, stack));
            ...
            if (stack.getItem() == SkillCards.QICE) {
                ...
                if (!offStack.isEmpty() && offStack.isIn(Tags.Items.CARD) && offStack.getCount() > 1) {
                    if (cd == 0) {
                        ...
                        inventory.setStack(18, stack); //用于在Handler中找到原本的stack
        ...
        if (!player.getWorld().isClient) {
            player.openHandledScreen(new ExtendedScreenHandlerFactory<>() {
                ...
                public Object getScreenOpeningData(ServerPlayerEntity player) {
                    return new OpenInvPayload(player.getUuid());
                }
    ...
    public static final ScreenHandlerType<SimpleMenuHandler> SIMPLE_MENU_HANDLER = Registry.register(Registries.SCREEN_HANDLER, "simple_menu", new ExtendedScreenHandlerType<>((syncId, inv, data) -> new SimpleMenuHandler(syncId, new SimpleInventory(20), inv.player.getWorld().getPlayerByUuid(data.uuid())), OpenInvPayload.OPENINV_CODEC));

    public static final ScreenHandlerType<PlayerInvScreenHandler> PLAYER_INV_SCREEN_HANDLER = Registry.register(Registries.SCREEN_HANDLER, "player_inv", new ExtendedScreenHandlerType<>((syncId, inv, data) -> new PlayerInvScreenHandler(syncId, new SimpleInventory(60), inv.player.getWorld().getPlayerByUuid(data.uuid())), OpenInvPayload.OPENINV_CODEC));

    public static final ScreenHandlerType<FullInvScreenHandler> FULL_INV_SCREEN_HANDLER = Registry.register(Registries.SCREEN_HANDLER, "full_inv", new ExtendedScreenHandlerType<>((syncId, inv, data) -> new FullInvScreenHandler(syncId, inv, new SimpleInventory(64), (LivingEntity) inv.player.getWorld().getEntityById(data.id())), FullInvPayload.FULL_INV_CODEC));
