        ...
        public void onHurt(ItemStack stack, LivingEntity entity, DamageSource source, float amount) {
            if (entity instanceof PlayerEntity player && player.isDead()) {
                ...
                if (new Random().nextFloat() >= (float) c /13) {
                    player.sendMessage(Text.translatable("buqu.tip1", c + 1).formatted(Formatting.GREEN), false);
                    setTag(stack, c + 1);
                    player.setHealth(1);
                } else player.sendMessage(Text.translatable("buqu.tip2").formatted(Formatting.RED), false);
            }
        }
    }
