...
public class Wu {
            ...
            if (user instanceof PlayerEntity player && player.isDead()) {
                ...
                if (chuang.isEmpty()) {
                    ...
                    if (!chuang.contains(card.rank.rank)) {
                        ...
                        if (chuang.split(", ").length < 3) player.sendMessage(Text.translatable("buqu.tip2").formatted(RED), false);
                    }
                }
                skill.setNbt(nbt);
            }
    ...
    public static class Fanjian extends SkillItem implements DabaoSwordCommand.CSkill {
        public Fanjian(Settings settings) {super(settings);}
        @Override
        public void addTip(Skill skill, List<Text> tooltip) {
            int cd = skill.getCD();
        ...
        public boolean activeSkill(PlayerEntity user, Skill skill, LivingEntity target) {
            ...
            if (target instanceof PlayerEntity player && countCards(user) > 0) { //有手牌才能发动
                ...
                player.sendMessage(Text.translatable("select_a_suit", fanjianText(user, player, Suit.Heart), fanjianText(user, player, Suit.Diamond), fanjianText(user, player, Suit.Spade), fanjianText(user, player, Suit.Club)), false);
                player.sendMessage(Text.translatable("fanjian.time.tip").formatted(BOLD), false);
                return true;
            }
            return false;
        }
        ...
        public void triggerSkill(LivingEntity entity, Skill skill, LivingEntity target, int value) {
            if (entity instanceof PlayerEntity user) {
                ...
                if (target instanceof PlayerEntity tp) tp.sendMessage(message, false);
                ...
                if (c.suit.ordinal() + 1 != value) {
                    ...
                    target.damage(world(entity), damageSource(user, DamageTypes.MAGIC), 6);
                }
                skill.setCD(20);
            }
        }
    ...
    public static class Fenyin extends SkillItem {
        public Fenyin(Settings settings) {super(settings);}
    ...
    public static class Gongxin extends SkillItem {
        ...
        public void onSlotClick(PlayerInvScreenHandler handler, PlayerEntity player, Skill skill, PlayerEntity target, int slot, int button, SlotActionType action) {
            ...
            target.sendMessage(Text.translatable("dabaosword.discard", player.getDisplayName(), target.getDisplayName(), selected.toHoverableText()), false);
    ...
    public static class Shixin extends SkillItem {
        public Shixin(Settings settings) {super(settings);}
    ...
    public static class Xiaoji extends SkillItem {
        public Xiaoji(Settings settings) {super(settings);}
        @Override
        public void addTip(Skill skill, List<Text> tooltip) {tooltip.add(getTip(GREEN));}
