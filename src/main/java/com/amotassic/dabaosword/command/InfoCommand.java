        ...
        if(component.isPresent()) {
            ...
            for(Pair<SlotReference, ItemStack> entry : allEquipped) {
                ...
                if(stack.getItem() instanceof SkillItem || stack.getItem() instanceof EquipmentItem) {
                    if (i == 0) {context.getSource().sendMessage(Text.literal(target.getNameForScoreboard()).append(Text.translatable("info.item")));}
                    context.getSource().sendMessage(stack.getName());
                    i++;
                }
            }
            if (i == 0) {context.getSource().sendMessage(Text.literal(target.getNameForScoreboard()).append(Text.translatable("info.none")));}
        } else {
            context.getSource().sendMessage(Text.literal(target.getNameForScoreboard()).append(Text.translatable("info.none")));
        }
        return 1;
    }
}
