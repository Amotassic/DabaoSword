...
public class InfoCommand {
    ...
    public static Inventory updateInv(Inventory inventory, LivingEntity target, boolean editable) {
        ...
        var optional = AccessoriesCapability.getOptionally(target);
        if (optional.isPresent()) { //饰品栏
            List<ItemStack> stacks = new ArrayList<>();
            optional.get().getContainers().values().forEach(a -> a.getAccessories().forEach(b -> stacks.add(b.getSecond())));
