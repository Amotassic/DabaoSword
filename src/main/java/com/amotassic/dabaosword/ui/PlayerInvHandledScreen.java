...
public class PlayerInvHandledScreen extends HandledScreen<PlayerInvScreenHandler> {
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        context.drawTexture(TEXTURE, x, y,0,0, backgroundWidth, 17);
        for (int i = 0; i < rows; i++) {
            context.drawTexture(TEXTURE, x, y + 17 + i * 18,0,17, backgroundWidth, 18);
        }
        context.drawTexture(TEXTURE, x, y + rows * 18 + 17,0,215, backgroundWidth, 7);
    }
    ...
    private Map<Integer, Integer> getClicks() {
        String str = handler.getSlot(82).getStack().getOrCreateNbt().getString("Clicks");
