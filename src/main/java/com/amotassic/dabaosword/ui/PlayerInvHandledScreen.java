...
public class PlayerInvHandledScreen extends HandledScreen<PlayerInvScreenHandler> {
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < rows; i++) {
            context.drawTexture(RenderPipelines.GUI_TEXTURED, TEXTURE, x, y + 17 + i * 18,0,17, backgroundWidth, 18, 256, 256);
        }
        context.drawTexture(RenderPipelines.GUI_TEXTURED, TEXTURE, x, y + rows * 18 + 17,0,215, backgroundWidth, 7, 256, 256);
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (!screenTips.isEmpty()) for (var text : screenTips) {
            ...
            // 绘制文本
            context.drawText(textRenderer, text, 2, y, -1, false);
        }
        ...
        getClicks().forEach((i, clicks) -> {
            ...
            context.drawText(this.textRenderer, Text.literal(clicks + "").formatted(Formatting.RED), slot.x, slot.y + 6, Colors.DARK_GRAY, false);
        });
    }
