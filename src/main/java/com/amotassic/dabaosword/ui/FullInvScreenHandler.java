    ...
    public FullInvScreenHandler(int syncId, PlayerInventory inv, PacketByteBuf buf) {
        ...
        addSlot(new Slot(inventory, 36, 8 + 4 * 18, 18 * armor) {
            public com.mojang.datafixers.util.Pair<Identifier, Identifier> getBackgroundSprite() {
                return com.mojang.datafixers.util.Pair.of(BLOCK_ATLAS_TEXTURE, EMPTY_HELMET_SLOT_TEXTURE);
            }
        });
        addSlot(new Slot(inventory, 37, 8 + 5 * 18, 18 * armor) {
            public com.mojang.datafixers.util.Pair<Identifier, Identifier> getBackgroundSprite() {
                return com.mojang.datafixers.util.Pair.of(BLOCK_ATLAS_TEXTURE, EMPTY_CHESTPLATE_SLOT_TEXTURE);
            }
        });
        addSlot(new Slot(inventory, 38, 8 + 6 * 18, 18 * armor) {
            public com.mojang.datafixers.util.Pair<Identifier, Identifier> getBackgroundSprite() {
                return com.mojang.datafixers.util.Pair.of(BLOCK_ATLAS_TEXTURE, EMPTY_LEGGINGS_SLOT_TEXTURE);
            }
        });
        addSlot(new Slot(inventory, 39, 8 + 7 * 18, 18 * armor) {
            public com.mojang.datafixers.util.Pair<Identifier, Identifier> getBackgroundSprite() {
                return com.mojang.datafixers.util.Pair.of(BLOCK_ATLAS_TEXTURE, EMPTY_BOOTS_SLOT_TEXTURE);
            }
        });
        addSlot(new Slot(inventory, 40, 8 + 8 * 18, 18 * armor) {
            public com.mojang.datafixers.util.Pair<Identifier, Identifier> getBackgroundSprite() {
                return com.mojang.datafixers.util.Pair.of(BLOCK_ATLAS_TEXTURE, EMPTY_OFFHAND_ARMOR_SLOT);
            }
        });
        for (i = 0; i < 5; i++) { //饰品栏
            for (j = 0; j < 9; j++) {
    ...
    private static Set<Integer> setup(Inventory inventory, LivingEntity target, boolean editable) {
        ...
        if (editable) {
            if (target instanceof PlayerEntity player) {
                ...
                var stacks = owner.getInventory().stacks;
