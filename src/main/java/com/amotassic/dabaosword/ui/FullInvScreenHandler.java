...
public class FullInvScreenHandler extends ScreenHandler {
    ...
    public FullInvScreenHandler(int syncId, PlayerInventory inv, SimplePayload buf) {
        ...
        List<Object[]> slotData = Arrays.asList(
                ...
                new Object[]{40, 8, EMPTY_OFF_HAND_SLOT_TEXTURE}
        ); //护甲和副手的格子
        for (Object[] data : slotData) {
            ...
            addSlot(new Slot(inventory, slotId, 8 + xOffset * 18, 18 * armor) {
                public Identifier getBackgroundSprite() {return texture;}
            });
        }
        addSlot(new Slot(inventory, TRINKET_INDEX - 2, 8 + 3 * 18, 18 * armor) {
            public Identifier getBackgroundSprite() {return Identifier.ofVanilla("container/slot/horse_armor");}
        }); // BodyArmor槽位
        addSlot(new Slot(inventory, TRINKET_INDEX - 1, 8 + 2 * 18, 18 * armor) {
            public Identifier getBackgroundSprite() {return Identifier.ofVanilla("container/slot/saddle");}
        }); // Saddle槽位
    ...
    public static class EntityInventory implements Inventory {
        ...
        public ItemStack getStack(int slot) {
            if (slotIndexes.contains(slot)) {
                ...
                if (slot == 41) return owner.getEquippedStack(EquipmentSlot.BODY);
                if (slot == 42) return owner.getEquippedStack(EquipmentSlot.SADDLE);
        ...
        public void setStack(int slot, ItemStack stack) {
            ...
            if (slot == 41) owner.equipStack(EquipmentSlot.BODY, stack);
            if (slot == 42) owner.equipStack(EquipmentSlot.SADDLE, stack);
            if (slot >= TRINKET_INDEX) {
                var pair = trinketsWithSlots(owner).get(slot - TRINKET_INDEX);
